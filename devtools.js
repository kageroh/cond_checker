...
chrome.devtools.network.onRequestFinished.addListener(function (request) {
	if (!/^http:\/\/.+?\/kcsapi\/api_(?:get_member\/ship(?:[23]|_deck)|port\/port)$/.test(request.request.url)) return;

	var ship2     = /\/ship2$/    .test(request.request.url);
	var ship3     = /\/ship3$/    .test(request.request.url);
	var ship_deck = /\/ship_deck$/.test(request.request.url);
